...
// var zthonTeam = ["stonemoney533","8bitbrigadier","ericptg","abra","ainkayes","animejessica","arcticlight","artsierosie","bestestcat","betsy7cat","brokensword9","brooky120","bushelofchewygoodness","captain_trina","choco","ctotheatherine","dannychipotle","darkdecay","daskippy","dinos","diredwarf","douggernaught","fartgarfunkle_","femtastic","fistyface","frosty10001","funnyman3595","gannon11","gemsmrt","greatscottlp","gymnast86","halfemptyetank","hipsterharbinger","insaneintherain","jacksonparodi","john_mackay13","justbluey","jyggy11","kainmoogle","kampydk","kellycelina","klinkit","kortraction","legosjedi","letsnarvik","lindsaypez","lizerdoo","lucarimew","maevedx","lyricofwisdom","marishkabob","marzakay","metalninjawolf","milotic","mojofibers","nytsura","oc1_","omgitsmonika","pandafloof","paulblart","phonic88","pingpong980","ppeach2010","retsam19","ridleypals","romscout","sab_irene","samanthanatalie","serperior","shado_temple","shoop631","silverdslite","simulatedgreg","smoople","soaringchris","starrlett20","forsakenpanda","stinusmeret","supermcgamer","surfimp","swchris","tbforgood","tekkirotaru","teracmusic","thatoneyetee","the8bitdrummer","thedivinebacon","thelyterage","drumultima","thepurplesteph","thezwolfenstein","thunderscott","trainertrevino","tylinos","valor6","vidya","waluigi","zqlina","zeldathonbot","zeldathonmods","ztechdesk","first_bot","dynomation","flyingludicolo","slamb0b","blockheadstudios","jb2448","irisangel","math_effect","zic3","skootish","melvaker","loveshorsesgirl","drthrex","brittanysaturn","dylanhatestwitch","itsthehutch","ptrbob","tabetaicooking","ynkebour","iylila","bonzthebard","surfimp","solaceanddread","okamihylia","tkasony","ablacksparrow","atsukunatsu"]
var kinstoneList = []
var guestsList = []
var houses = []
var houseTeamList = []
var houseUserList = []
chrome.runtime.sendMessage({ request: "people" });
			...
			if (chatLine != undefined) {
				if (chatLine.querySelectorAll('span.text-fragment').length > 0) {
					try {
						...
						//https://i.imgur.com/9XxOpco.png
						if (kinstoneList.includes(username)) {
							if(houseUserList.includes(username)) {
								//Merge badge
								let i = houseTeamList.findIndex((user)=>{return user.name == username})
								if(houseTeamList[i]) {
									let houseId = houseTeamList[i].house_id
									if (!ffz) {
										badgeCollection.insertAdjacentElement("afterbegin", createChatBadge("badge-team", "Kinstone Team ("+houses[houseId]+")",'color-'+houseId));
									} else {
										var ffzBadgeCollection = chatLine.querySelector("span.chat-line__message--badges")
										ffzBadgeCollection.insertAdjacentElement("afterBegin", createChatBadge("badge-team", "Kinstone Team ("+houses[houseId]+")",'color-'+houseId));
									}
								}
							} else {
								if (!ffz) {
									badgeCollection.insertAdjacentElement("afterbegin", createChatBadge("badge-team", "Kinstone Team"));
								} else {
									var ffzBadgeCollection = chatLine.querySelector("span.chat-line__message--badges")
									ffzBadgeCollection.insertAdjacentElement("afterBegin", createChatBadge("badge-team", "Kinstone Team"));
								}
							}
						} else if (guestsList.includes(username)) {
							if(houseUserList.includes(username)) {
								//Merge badge
								let i = houseTeamList.findIndex((user)=>{return user.name == username})
								if(houseTeamList[i]) {
									let houseId = houseTeamList[i].house_id
									if (!ffz) {
										badgeCollection.insertAdjacentElement("afterbegin", createChatBadge("badge-guest", "Event Guest ("+houses[houseId]+")",'color-'+houseId));
									} else {
										var ffzBadgeCollection = chatLine.querySelector("span.chat-line__message--badges")
										ffzBadgeCollection.insertAdjacentElement("afterBegin", createChatBadge("badge-guest", "Event Guest ("+houses[houseId]+")",'color-'+houseId));
									}
								}
							} else {
								if (!ffz) {
									badgeCollection.insertAdjacentElement("afterbegin", createChatBadge("badge-guest", "Event Guest"));
								} else {
									var ffzBadgeCollection = chatLine.querySelector("span.chat-line__message--badges")
									ffzBadgeCollection.insertAdjacentElement("afterBegin", createChatBadge("badge-guest", "Event Guest"));
								}
							}
						} else if (houseUserList.includes(username)) {
							let i = houseTeamList.findIndex((user)=>{return user.name == username})
							if(houseTeamList[i]) {
								let houseId = houseTeamList[i].house_id
								if (!ffz) {
									badgeCollection.insertAdjacentElement("afterbegin", createChatBadge("badge-"+houses[houseId], "Team "+houses[houseId],'color-'+houseId));
								} else {
									var ffzBadgeCollection = chatLine.querySelector("span.chat-line__message--badges")
									ffzBadgeCollection.insertAdjacentElement("afterBegin", createChatBadge("badge-"+houses[houseId], "Team "+houses[houseId],'color-'+houseId));
								}
							}
						}
					} catch (err) {
					...
					// Bless Remover Depricated because nobody spams bless any more
...
function donatePage() {
	var newwindow = window.open("http://donate.zeldathon.com/", 'name', 'height=600,width=400,titlebar=no,menubar=no,location=yes,left=10');
	if (window.focus) { newwindow.focus() }
}

//create a chat badge, matching the CSS of twitch
function createChatBadge(subimg, subtext, teamcolor) {
	var badge = document.createElement("a")
	badge.setAttribute("data-a-target", "chat-badge")
	badge.setAttribute("target", "_blank")
	badge.classList.add("tooltip")
	badge.classList.add(subimg)
	if(teamcolor) {
		badge.classList.add(teamcolor)
		// badge.href = "https://house.stone.money/"
	}
	badge.classList.add("ffz-badge-zdc")
	var tooltiptext = document.createElement("span")
	//tw-tooltip tw-tooltip--align-left tw-tooltip--up
	
	tooltiptext.classList.add("tooltiptext")
	tooltiptext.style.marginRight = "1rem"
	tooltiptext.innerText = subtext
	badge.insertAdjacentElement("beforeend", tooltiptext)
	return badge;
}
